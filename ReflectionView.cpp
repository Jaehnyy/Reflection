...
BEGIN_MESSAGE_MAP(CReflectionView, CFormView)
	...
	ON_BN_CLICKED(IDC_CHECK1, &CReflectionView::OnBnClickedCheck1)
...
void CReflectionView::OnBnClickedCheck1()
{
	BOOL bCheck = m_check1.GetCheck();
	if (bCheck == FALSE)
	{
		if (::IsWindow(m_list.GetSafeHwnd()) != FALSE)
			m_list.UnsubclassWindow();

		m_list2.SubclassDlgItem(IDC_LIST1, this);
	}
	else
	{
		if (::IsWindow(m_list2.GetSafeHwnd()) != FALSE)
			m_list2.UnsubclassWindow();
